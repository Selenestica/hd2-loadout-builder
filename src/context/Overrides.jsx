  ...
  const handleUpdate = (store, updatedObject) => {
    updateObject(store, updatedObject).then(() => {
      setOverrides((prev) => {
        const updated =
          prev[store]?.map((x) =>
            x.id === updatedObject.id ? updatedObject : x,
          ) || [];
        const newObj = { ...prev };
  ...
  const getFinalData = useCallback(
    (id, type) => {
      if (
        ...
        const defaultData = structuredClone(
          strategemData.find((x) => x.id === id),
        );
        const foundOverride = structuredClone(
          overrides.stratOverrides?.find((x) => x.id === id),
        );
        ...
        const defaultData = structuredClone(
          primaryWeaponData.find((x) => x.id === id),
        );
        const foundOverride = structuredClone(
          overrides.primaryOverrides?.find((x) => x.id === id),
        );
        return !!foundOverride
          ? {
              ...defaultData,
        ...
        const defaultData = structuredClone(
          secondaryWeaponData.find((x) => x.id === id),
        );
        const foundOverride = structuredClone(
          overrides.secondaryOverrides?.find((x) => x.id === id),
        );
        return !!foundOverride
          ? {
              ...defaultData,
        ...
        const defaultData = structuredClone(
          grenadeData.find((x) => x.id === id),
        );
        const foundOverride = structuredClone(
          overrides.grenadeOverrides?.find((x) => x.id === id),
        );
        return !!foundOverride
          ? {
              ...defaultData,
        ...
        return structuredClone(boosterData.find((x) => x.id === id));
      }
    },
    [
      strategemData,
