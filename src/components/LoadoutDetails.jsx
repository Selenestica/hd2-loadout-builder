...
export default function LoadoutDetails({ ...props }) {
  const { selectedLoadout, setSelectedLoadout, setLoadouts, loadouts } =
    useContext(LoadoutsContext);
  ...
  const generateRandomLoadout = useCallback(() => {
    ...
    while (true) {
      ...
      const containsMultipleBackpacks =
        [attempt.strat1, attempt.strat2, attempt.strat3, attempt.strat4]
          .map((x) => x.hasBackpack)
          .filter((x) => !!x).length > 1;
      if (containsMultipleBackpacks) continue;
      // strats may not contain multiple support slot consumers
      const containsMultipleSupportSlots =
        [attempt.strat1, attempt.strat2, attempt.strat3, attempt.strat4]
          .map((x) => x.supportSlotNecessary)
          .filter((x) => !!x).length > 1;
  ...
  const newStrat1 = newLoadout.strat1
    ? getFinalData(newLoadout.strat1, "strat1")
    : null;
  const newStrat2 = newLoadout.strat2
    ? getFinalData(newLoadout.strat2, "strat2")
    : null;
  const newStrat3 = newLoadout.strat3
    ? getFinalData(newLoadout.strat3, "strat3")
    : null;
  const newStrat4 = newLoadout.strat4
    ? getFinalData(newLoadout.strat4, "strat4")
    : null;
  const newPrimary = newLoadout.primary
    ? getFinalData(newLoadout.primary, "primary")
    : null;
  const newSecondary = newLoadout.secondary
    ? getFinalData(newLoadout.secondary, "secondary")
    : null;
  const newGrenade = newLoadout.grenade
    ? getFinalData(newLoadout.grenade, "grenade")
    : null;
    ...
    <>
      ...
      >
        ...
        >
          <input
            type="text"
            placeholder="Loadout Name (click to edit)"
            className={css`
              align-self: center;
              text-align: center;
              width: 100%;
          ...
          <StyledSelect
            value={
              factionOptions.find((o) => o.value === newLoadout.faction) ??
              factionOptions.find((o) => o.value === "Generic")
            }
            onChange={(val) =>
              setNewLoadout((old) => ({ ...old, faction: val.value }))
            }
        ...
        <StrategemDetails
          strat={newStrat1}
          active={selectedTarget.target === "strat1"}
          reset={() => {
        ...
        <StrategemDetails
          strat={newStrat2}
          active={selectedTarget.target === "strat2"}
          reset={() => {
        ...
        <StrategemDetails
          strat={newStrat3}
          active={selectedTarget.target === "strat3"}
          reset={() => {
        ...
        <StrategemDetails
          strat={newStrat4}
          active={selectedTarget.target === "strat4"}
          reset={() => {
        ...
        <PrimaryDetails
          primary={newPrimary}
          active={selectedTarget.target === "primary"}
          reset={() => {
        ...
        <SecondaryDetails
          ...
          active={selectedTarget.target === "secondary"}
          reset={() => {
        ...
        <GrenadeDetails
          grenade={newGrenade}
          active={selectedTarget.target === "grenade"}
          reset={() => {
        ...
        <ArmorDetails
          armor={newArmor}
          active={selectedTarget.target === "armor"}
          reset={() => {
        ...
        >
          ...
          >
            ...
            {confirmDelete && <div onClick={handleDelete}>Confirm?</div>}
          </button>
          <ShareButton
            className={css`
              padding: 0.2em;
