...
export default function LoadoutList({ ...props }) {
  const {
    setSelectedLoadout,
    selectedLoadout,
    loadouts,
  ...
  const sortedLoadouts = useMemo(() => {
    return loadouts
      ...
      .filter(
        (l) =>
          l.faction === loadoutFactionFilter || loadoutFactionFilter === "",
      );
        ...
        >
          ...
          >
            ...
            <button
              className={css`
                flex-grow: 1;
                ${loadoutFactionFilter === "Illuminate"
                  ? "border-bottom: 3px solid yellow;"
                  : ""}
              `}
